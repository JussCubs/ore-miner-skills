...
| `REFINORE_API_KEY` | Yes | API key from refinORE Settings |
| `REFINORE_AUTH_TOKEN` | Alt | Legacy JWT token (use API key instead) |
| `WEBHOOK_URL` | No | Webhook URL for event reporting |
...
curl -X POST "$REFINORE_API_URL/mining/start" \
  ...
  -d '{"sol_amount":0.005,"num_squares":25,"tile_selection_mode":"optimal","risk_tolerance":"medium","mining_token":"SOL","auto_restart":true,"frequency":"every_round"}'
```
...
# Round history
curl -s "$REFINORE_API_URL/mining/session-rounds" -H "x-api-key: $REFINORE_API_KEY"
```
...
| `POST` | `/mining/start-strategy` | Start with advanced config (custom tile IDs) |
| `POST` | `/mining/stop` | Stop active session |
| `POST` | `/mining/reload-session` | Reload with existing config |
| `GET` | `/mining/session` | Get active session status |
| `GET` | `/mining/session-rounds` | Get round-by-round results |
| `GET` | `/mining/history?limit=N` | Get historical sessions |
| `GET` | `/mining/last-config` | Get last mining config |
| `GET` | `/mining/round/:roundNumber` | Get specific round details |

### Round Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/rounds/current` | Current round info (EV, ML, miners, tiles) |

### Strategy Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/auto-strategies` | List saved strategies |
| `POST` | `/api/auto-strategies` | Create a strategy |
| `PUT` | `/api/auto-strategies/:id` | Update a strategy |
| `DELETE` | `/api/auto-strategies/:id` | Delete a strategy |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/auto-swap-orders` | List active orders |
| `POST` | `/api/auto-swap-orders` | Create DCA or limit order |
| `PUT` | `/api/auto-swap-orders/:id` | Update order |
| `DELETE` | `/api/auto-swap-orders/:id` | Cancel order |

DCA example:
```json
{"type":"dca","input_token":"USDC","output_token":"ORE","amount":5,"interval_hours":24,"total_orders":30}
```
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/staking/info` | Stake info (stORE balance, APR ~22%) |

### Wallet

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/wallet/balance` | All token balances (SOL, ORE, USDC, stORE, SKR) |
| `GET` | `/api/wallet/transactions` | Transaction history |

### SSE (Real-Time Events)

```bash
curl -N "$REFINORE_API_URL/sse" -H "x-api-key: $REFINORE_API_KEY" -H "Accept: text/event-stream"
```
Events: `round_start`, `round_end`, `deployment`, `claim`, `balance_update`

### Public Endpoints (No Auth)

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/refinore-apr` | Current staking APR and market data |
